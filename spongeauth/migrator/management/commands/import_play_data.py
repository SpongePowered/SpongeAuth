            ...
            for muser in migrator.models.User.objects.all():
                ...
                if muser.is_totp_confirmed:
                    twofa.models.TOTPDevice(
                        ...
                        drift=0,
                        activated_at=now,
                        base32_secret=muser.totp_secret).save()
                    auser.totp_enabled = True
                if muser.avatar_url:
                    avatar = accounts.models.Avatar(
                        user=auser,
                        remote_url=muser.avatar_url,
                        source=accounts.models.Avatar.URL)
                    avatar.save()
                    auser.current_avatar = avatar
                if muser.google_id:
                    accounts.models.ExternalAuthenticator(
                        user=auser,
                        source=accounts.models.ExternalAuthenticator.GOOGLE,
                        external_id=muser.google_id).save()
                auser.save()

                accounts.models.User.objects.filter(id=muser.id).update(
                    joined_at=muser.join_date)
