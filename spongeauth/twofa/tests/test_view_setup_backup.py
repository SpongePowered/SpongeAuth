...
class TestSetupBackup(django.test.TestCase):
    ...
    def path(self, device=None, device_id=None):
        return django.shortcuts.reverse("twofa:paper-code", kwargs={"device_id": device_id or device.id})
    ...
    def test_only_own_devices(self):
        ...
        other_user.save()
        other_device = models.PaperDevice(owner=other_user)
        other_device.save()
    ...
    def test_forbids_active_device(self):
        active_device = models.PaperDevice(owner=self.user, activated_at=timezone.now())
        active_device.save()
    ...
    def test_forbids_deleted_device(self):
        deleted_device = models.PaperDevice(owner=self.user, deleted_at=timezone.now())
        deleted_device.save()
