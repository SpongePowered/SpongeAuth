...
def validate_username(username):
    ...
    if re.search(r"\W", username[0]):
        errs.append(
            ValidationError(
                _("Username must begin with a number, letter or underscore."), code="username_initial_charset"
            )
        )
    if re.search(r"[^A-Za-z0-9]", username[-1]):
        errs.append(ValidationError(_("Username must end with a letter or number."), code="username_ending_charset"))
    if re.search(r"[-_.]{2,}", username):
        errs.append(
            ValidationError(
                _("Username must not contain two special characters in a row."), code="username_double_special"
            )
        )
    if re.search(r"\.(js|json|css|htm|html|xml|jpg|jpeg|png|gif|bmp|ico|tif|tiff|woff)$", username):
        errs.append(
            ValidationError(_("Username must not end with a confusing file suffix."), code="username_file_suffix")
        )
    if errs:
...
class Group(models.Model):
    ...
    internal_name = models.CharField(
        max_length=20, unique=True, blank=False, null=False, validators=[validate_username]
    )
...
class User(AbstractBaseUser):
    username = models.CharField(max_length=20, unique=True, blank=False, null=False, validators=[validate_username])
    ...
    current_avatar = models.ForeignKey("Avatar", null=True, blank=True, related_name="+", on_delete=models.SET_NULL)

    twofa_enabled = models.BooleanField(default=False, null=False, verbose_name=_("2FA Enabled"))
    ...
    def must_agree_tos(self):
        return TermsOfService.objects.filter(current_tos=True).exclude(agreed_users=self)

    def _test_agree_all_tos(self):
        if not self.pk:
            return
        for tos in self.must_agree_tos():
            TermsOfServiceAcceptance(tos=tos, user=self).save()
...
def _avatar_upload_path(instance, filename):
    ...
    bits = ("avatars", filehash[0:2], filehash[2:4], filehash[4:6], ("{}.{}".format(filehash[6:], ext)))
...
class Avatar(models.Model):
    UPLOAD = "upload"
    URL = "url"
    AVATAR_CHOICES = ((UPLOAD, _("Uploaded avatar")), (URL, _("Avatar at URL")))

    user = models.ForeignKey(User, on_delete=models.CASCADE)
    added_at = models.DateTimeField(auto_now_add=True, blank=False, null=False)

    image_file = models.ImageField(null=True, blank=True, upload_to=_avatar_upload_path)
    remote_url = models.URLField(null=True, blank=True)

    source = models.CharField(max_length=10, choices=AVATAR_CHOICES, default=UPLOAD, blank=False, null=False)
...
class ExternalAuthenticator(models.Model):
    GOOGLE = "google"
    SOURCE_CHOICES = ((GOOGLE, _("Google")),)

    user = models.ForeignKey(User, on_delete=models.CASCADE)

    source = models.CharField(max_length=20, choices=SOURCE_CHOICES, default=GOOGLE, blank=False, null=False)

    external_id = models.CharField(max_length=255, blank=False, null=False)

    def __str__(self):
        return "{} credential for user {}".format(self.get_source_display(), self.user_id)
...
class TermsOfServiceAcceptance(models.Model):
    ...
    tos = models.ForeignKey(TermsOfService, null=False, blank=False, on_delete=models.CASCADE)
    accepted_at = models.DateTimeField(auto_now_add=True, null=False, blank=False)

    def __str__(self):
        return "TermsOfServiceAcceptance: {} accepted {} at {}".format(self.user, self.tos, self.accepted_at)
