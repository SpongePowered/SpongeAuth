...
class AdminUserChangeForm(forms.ModelForm):
    ...
    groups = forms.ModelMultipleChoiceField(
        queryset=models.Group.objects.all(), required=False, widget=widgets.FilteredSelectMultiple("groups", False)
    )
...
class UserAdmin(django.contrib.auth.admin.UserAdmin):
    ...
    fieldsets = (
        ...
        ("Groups", {"classes": ("collapse",), "fields": ("groups",)}),
        ("Profile fields", {"classes": ("collapse",), "fields": ("mc_username", "irc_nick", "gh_username")}),
    )
    filter_horizontal = ()
...
class GroupAdminForm(forms.ModelForm):
    ...
    users = forms.ModelMultipleChoiceField(
        ...
        widget=autocomplete.ModelSelect2Multiple(
            url="accounts:users-autocomplete", attrs={"data-minimum-input-length": 3}
        ),
        required=False,
    )
...
class TermsOfServiceAdminForm(forms.ModelForm):
    class Meta:
        ...
        fields = ("name", "tos_url", "tos_date", "current_tos")
...
class TermsOfServiceAdmin(admin.ModelAdmin):
    ...
    def get_readonly_fields(self, request, obj=None):
        if not obj:
            return self.readonly_fields
        return self.readonly_fields + ("name", "tos_date", "tos_url")
