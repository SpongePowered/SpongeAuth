...
class CoreFieldsMixin(forms.Form):
    ...
    email = forms.EmailField(
        ...
        help_text=_('Never displayed publicly'))
...
class ProfileFieldsMixin(forms.Form):
    ...
    irc_nick = forms.CharField(
        label=_('IRC Nick'), max_length=255, required=False,
        help_text=_('Join us on IRC often? Enter your IRC nick here'))
    gh_username = forms.CharField(
        label=_('GitHub Username'), max_length=255, required=False,
        help_text=_('Publish your software on GitHub? '
                    'Enter your GitHub username here'))
    discord_id = forms.CharField(
        label=_('Discord ID'), max_length=255, required=False,
        help_text=_('You\'re using Discord?'
...
class ForgotPasswordForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            Field('email'),
            Submit('reset', _("Send me that link!"), css_class='pull-right'),
        )


class ForgotPasswordSetForm(forms.Form):
    password = forms.CharField(
        label=_('New Password'), min_length=8, max_length=255,
...
class SetAvatarForm(forms.Form):
    ...
    avatar_from = forms.ChoiceField(
        choices=SOURCE_CHOICES, widget=forms.RadioSelect())
    ...
    def clean(self):
        ...
        if avatar_from == self.UPLOAD and not avatar_image:
            self.add_error('avatar_image', forms.ValidationError(
