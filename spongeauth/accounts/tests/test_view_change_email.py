...
class TestChangeEmail(django.test.TestCase):
    def setUp(self):
        self.user = factories.UserFactory.create(email_verified=False)
...
class TestChangeEmailStep2(django.test.TestCase):
    ...
    def test_bad_user_id(self):
        resp = self.client.get(
            self.path(None, "deadbeef-cafe", uidb64="foo", new_email=urlsafe_base64_encode(b"a@example.com"))
        )
        assert resp.status_code == 400
    ...
    def test_activates_if_not_active(self):
        ...
        resp = self.client.get(self.path(self.user, "deadbeef-cafe", new_email=urlsafe_base64_encode(b"a@example.com")))
        assert resp.status_code == 302
