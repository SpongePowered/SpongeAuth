...
class TestProfile(django.test.TestCase):
    ...
    def test_get(self):
        ...
        assert set(resp.context.keys()).issuperset({"profile_form", "password_form", "avatar_form", "user"})

    def test_updates_profile_on_save(self):
        resp = self.client.post(
            self.path(),
            {"form": "profile", "mc_username": "mcloving", "irc_nick": "ISeekYou", "gh_username": "CodyMcCodeFace"},
        )
        assert resp.status_code == 302
    ...
    def test_invalid_profile(self):
        resp = self.client.post(
            self.path(),
            {"form": "profile", "mc_username": "z" * 256, "irc_nick": "ISeekYou", "gh_username": "CodyMcCodeFace"},
        )
        assert resp.status_code == 200
        user = models.User.objects.get(id=self.user.id)
    ...
    def test_updates_password_on_save(self):
        ...
        resp = self.client.post(
            self.path(), {"form": "password", "old_password": "secret", "new_password": "slartibartfast"}
        )
        assert resp.status_code == 302
        assert resp["Location"] == self.path()
        user = models.User.objects.get(id=self.user.id)
    ...
    def test_incorrect_old_password(self):
        resp = self.client.post(
            self.path(), {"form": "password", "old_password": "sekrit", "new_password": "slartibartfast"}
        )
        assert resp.status_code == 200
        user = models.User.objects.get(id=self.user.id)
    ...
    def test_updates_avatar_on_save(self):
        ...
        # set to gravatar
        resp = self.client.post(self.path(), {"form": "avatar", "avatar_from": "gravatar"})
        user = models.User.objects.get(id=self.user.id)
        assert user.current_avatar.get_absolute_url().startswith("https://www.gravatar.com/")
        assert resp.status_code == 302

        # set to letter
        resp = self.client.post(self.path(), {"form": "avatar", "avatar_from": "letter"})
        user = models.User.objects.get(id=self.user.id)
        assert user.current_avatar is None
        assert resp.status_code == 302

    def test_select_upload_avatar_without_avatar(self):
        resp = self.client.post(self.path(), {"form": "avatar", "avatar_from": "upload"})
        assert resp.status_code == 200
        user = models.User.objects.get(id=self.user.id)
        assert user.current_avatar is None
