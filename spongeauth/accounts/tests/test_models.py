...
class TestUserManager:
    ...
    def test_create_superuser(self):
        user = models.User.objects.create_superuser(username="foo", email="foo@example.com", password="exciting")
        assert isinstance(user, models.User)
        assert user.username == "foo"
        assert user.email == "foo@example.com"
    ...
    def test_get_user_caseinsensitively(self):
        user = models.User.objects.create_user(
            username="foo", email="foo@example.com", password="exciting", mc_username="bar"
        )
...
class TestAvatar:
    ...
    def test_get_absolute_url_upload_invalid(self):
        ...
        avatar = factories.AvatarFactory.build(user=user, source=models.Avatar.UPLOAD, remote_url="")
    ...
    def test_get_absolute_url_remote_empty(self):
        user = factories.UserFactory.build(username="foobar")
        fallback = letter_avatar.LetterAvatar("foobar").get_absolute_url()
        avatar = factories.AvatarFactory.build(user=user, source=models.Avatar.URL, remote_url="")
        assert avatar.get_absolute_url() == fallback
...
class TestExternalAuthenticator:
    def test_str(self):
        assert (
            str(
                models.ExternalAuthenticator(user_id=21, source=models.ExternalAuthenticator.GOOGLE, external_id="blah")
            )
            == "Google credential for user 21"
        )
