...
@pytest.mark.parametrize("size,out_filename", [("210x210", "input.png"),
                                               ("210", "input.png"),
                                               ("zzrot", "120x120.png"),
                                               ("100x100", "100x100.png"),
                                               ("100x50", "100x50.png"),
...
def test_avatar_for_user_upload(size, out_filename):
    ...
    with unittest.mock.patch.object(views,
                                    'get_object_or_404') as get_object_or_404:
...
@pytest.mark.parametrize("size,out_s", [("210x210", "210"), ("210", "210"),
                                        ("zzrot", "120"), ("100x100", "100"),
                                        ("100x50", "100"), ("50x100", "100"),
                                        ("1024x1024", "240"),
                                        ("2048x1024", "240"),
                                        ("1024x2048", "240")])
def test_avatar_for_user_gravatar(size, out_s):
    ...
    user.avatar = models.Avatar(
        source=models.Avatar.URL, remote_url='https://example.com/foo.png')
    with unittest.mock.patch.object(views,
                                    'get_object_or_404') as get_object_or_404:
        get_object_or_404.return_value = user
        resp = views.avatar_for_user(request, 'foo')
