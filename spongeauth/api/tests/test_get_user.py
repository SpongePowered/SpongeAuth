...
def test_invalid_api_key(client, fake):
    ...
    resp = client.get(
        django.shortcuts.reverse("api:users-detail", kwargs={"username": fake.user_name()}), {"apiKey": "foobar"}
    )
...
def test_existing_user(client, fake):
    ...
    resp = client.get(
        django.shortcuts.reverse("api:users-detail", kwargs={"username": user.username}), {"apiKey": "foobar"}
    )
...
def test_deleted_user(client, fake):
    ...
    user = accounts.tests.factories.UserFactory.create(is_active=False, deleted_at=fake.date_time_this_century())
    resp = client.get(
        django.shortcuts.reverse("api:users-detail", kwargs={"username": user.username}), {"apiKey": "foobar"}
    )
...
def test_existing_user_in_group(client, fake):
    ...
    assert data["groups"][0] == {"id": group.id, "name": group.name}
