...
def _require_api_key(fn):
    ...
    def _wrap(request, *args, **kwargs):
        ...
        if (not api_key
                or not api.models.APIKey.objects.filter(key=api_key).exists()):
...
def _encode_user(request, user):
    return {
        ...
        'email': user.email,
        'avatar_url':
        request.build_absolute_uri(user.avatar.get_absolute_url()),
        'groups':
        [_encode_group(request, group) for group in user.groups.all()],
    }
...
def _delete_user(request):
    ...
    user = get_object_or_404(
        accounts.models.User, is_active=True, username=username)
